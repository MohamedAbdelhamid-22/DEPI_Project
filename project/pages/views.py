...
# Signup Process with Custom Logic
...
def end_challenge(request, challenge_id):
    try:
        ...
        if challenge.is_ended:
            messages.error(request, "This challenge has already ended.")
        ...
        messages.success(request, "Challenge ended and rewards have been distributed successfully!")
        return redirect('end_challenge_page')
...
def submit_prediction(request, challenge_id):
    ...
    if request.method == 'POST':
        ...
        if ChallengeResult.objects.filter(user=user, challenge=challenge).exists():
            messages.error(request, "You have already submitted a prediction for this challenge.")
...
def end_challenge(request, challenge_id):
    try:
        ...
        messages.success(request, "Challenge ended and rewards distributed.")
        return redirect('end_challenge_page')
...
def demand_admin(request):
    if request.method == 'POST':
        try:
            ...
            messages.error(request, f"Demand prediction error: {e}")
